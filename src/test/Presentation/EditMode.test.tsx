...
import { TableContainer, Table, Tbody, ChakraProvider } from "@chakra-ui/react";

afterEach(() => {
  cleanup();
});

describe("EditModeに関するテスト", () => {
  test("handleChangeが呼ばれている", () => {
  ...
  test("isFilled=trueにすると赤い強調表示がなくなる", () => {
    const item = initialItem(0);
    const handleChange = jest.fn();
    const handleUpdate = jest.fn();
    render(
